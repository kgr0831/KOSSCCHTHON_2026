...
export function AuthGate({ children }: { children: React.ReactNode }) { const { ready, user } = useAuth(); if (!ready) return <Loading />; if (!user) return <Empty title="당신의 다음 이야기를 시작해 보세요" description="학교 이메일로 로그인하고 동문들과 경험을 나눠요." action={<Link className="button primary" href="/auth">학교 이메일로 시작하기 <ArrowRight size={16} /></Link>} />; return <>{children}</>; }
