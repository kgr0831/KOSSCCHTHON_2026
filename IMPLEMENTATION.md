...
| 디자인 선택·레퍼런스 이미지 | Design MD 자동 탐색, 기본 PNG 예시, 개인 MD 업로드·DB 저장·Claude 예시 생성 작업·PC/모바일 PNG·재시도. Studio Journal 스타일 추가 |
...
- 백엔드 28개 테스트와 Ruff 통과. 응답 전 DB 커밋, 개인 MD 격리·실패 재시도, 중첩 프로젝트 영역 보존 포함.
- 프런트 프로덕션 빌드·TypeScript 검사 및 Playwright 18개 통과.
...
- 실서비스 학교 이메일·회사 이메일에는 SMTP 설정이 필요합니다. 운영 학교 인증 권한 게이트 및 보조 인증 정책은 별도 검토 대상입니다.
- GitHub OAuth·저장소 동기화, 외부 URL 수집, Supabase Storage·운영 PostgreSQL·배포 구성은 이번 로컬 구현에 포함하지 않았습니다.
