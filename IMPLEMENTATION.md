...
| 구독 CLI | PC BAT 전용 Codex 기본·Claude 선택, API/CLI/혼합. DB에는 경로·계정 이메일·로그인 상태·모델·확인 PC/시각만 저장. 배포 웹은 API 생성과 연결 기록 조회 |
| UI 드래그 | 메뉴·버튼 등 UI 텍스트 선택과 이미지·링크 드래그 차단. 코드·입력창·소개 본문은 선택 허용. 스크롤 유지 |
...
| 위치 | 책임 |
| --- | --- |
| `backend/app/ai.py`, `ai_routes.py`, `cli_metadata.py`, `local_runtime.py`, `cli_login.py`, `cli_runner.py` | 서버 API, CLI 공식 상태 조회·메타데이터 저장·PC별 작업 분리·제한된 실행 |
...
- 백엔드 62개 테스트 통과, PostgreSQL 전용 테스트 1개는 기본 실행에서 생략. Ruff 통과. 최신 CLI 변경과 재시도 경계의 [Linux/실제 PG/무료 런타임 CI](https://github.com/kgr0831/KOSSCCHTHON_2026/actions/runs/35458594798) 성공. 스타일 재시도는 현재 요청한 실행 환경으로 배정하며 관련 4개 검사 통과.
- TypeScript·프로덕션 빌드 통과. 기존 Playwright 전체 29개 통과 이후, CLI PC/배포 설정 2개와 UI 선택·이미지 드래그·소개/코드 복사·스크롤 유지 1개 추가 검사 통과.
...
- 국민대 API 인증·생성 성공. Codex CLI 0.155.1 공식 계정·모델 조회와 gpt-5.6-sol 실제 JSON 생성 성공. Claude 구현 유지, 사용자 구독 해지로 현재 Claude 생성은 미검증.
- 무료 운영 앱에서 Google 로그인·로그아웃·재로그인, 같은 Google 계정의 별도 숭실대 이메일 인증, 비공개 포트폴리오 생성과 DB 저장 완료. 학적·학과·경력을 추정해 추가하지 않음.
- CLI 메타데이터 마이그레이션을 Supabase에 적용하고 본인 계정의 Codex·Claude 경로/이메일/상태/모델 기록 저장 및 별도 세션 재조회를 확인. 저장 설정으로 Codex 생성도 성공. 무료 배포의 새 CLI API 스키마, 프런트와 API health 200·DB ok 확인.
...
- Google Supabase Auth 및 같은 계정의 별도 학교 이메일 인증을 운영 환경에서 검증했습니다. 순천향대(sch.ac.kr)·국민대(kookmin.ac.kr)·숭실대(soongsil.ac.kr)를 지원하며 실제 메일 수신·링크 소비는 사용자 본인의 숭실대 주소로 확인했습니다.
- 사용자 결정으로 CLI는 PC 실행 전용입니다. 배포 웹의 PC CLI 원격 호출은 구현 범위에서 제외하며 API 모드를 사용합니다.
...
- 실제 모바일 기기 검증은 별도입니다. 운영 배포의 인증·학교 메일·AI 문서 저장은 검증했습니다.
