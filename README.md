...
**CLI는 항상 사용자의 PC에서만 실행**합니다. 배포 웹의 `AI 연결 설정`에서 일회성 PC 연결 코드를 만들고, Codex 또는 Claude에 로그인한 PC의 프로젝트 폴더에서 표시된 `start-local.bat --connector ...` 명령을 실행하세요. 열린 커넥터 창은 그 PC에 지정된 문서 작업만 임대해 실행하고 결과만 서버에 저장합니다. 연결 코드는 10분·한 번만 유효하며, 커넥터 인증 토큰은 PC 메모리에만 유지됩니다. 서버·DB·브라우저에는 CLI 로그인 정보나 커넥터 토큰 원문을 저장하지 않습니다. 커넥터를 닫거나 웹에서 연결을 해제하면 새 작업은 실행되지 않습니다.

로컬 앱의 실행 터미널에서 `X`는 Codex, `C`는 Claude 로그인을 엽니다. 배포 웹에서는 **PC 연결 코드 만들기 → PC에서 명령 실행 → 연결 상태 새로고침 → PC CLI·모델 선택 → 연결 설정 저장**을 진행합니다. DB에는 실행 경로·계정 이메일·마지막 로그인 확인 상태·모델·확인 PC와 시각만 저장합니다. 로그인 확인은 구독 잔여량이나 생성 권한 보장을 뜻하지 않습니다. 다른 PC 또는 CLI 계정으로 바뀌면 다시 연결해야 합니다.
...
- 백엔드 테스트 62개 통과. PostgreSQL 전용 테스트 1개는 기본 실행에서 생략하고 [Linux CI](https://github.com/kgr0831/KOSSCCHTHON_2026/actions/runs/35458594798)에서 실제 PG 마이그레이션·권한·이전, 프로덕션 이미지 및 무료 서버 사양 검사를 통과했습니다. TypeScript·프로덕션 빌드·Ruff도 통과했습니다.
